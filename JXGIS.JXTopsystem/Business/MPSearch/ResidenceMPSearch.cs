    ...
    public class ResidenceMPSearch
    {
        ...
        public static Dictionary<string, object> SearchResidenceMP(int PageSize, int PageNum, string DistrictID, string CommunityName, string ResidenceName, string AddressCoding, string PropertyOwner, string StandardAddress, int UseState)
        {
            ...
            using (var dbContext = SystemUtils.NewEFDbContext)
            {
                ...
                {
                    data1 = query.OrderBy(t => t.CountyID).OrderBy(t => t.NeighborhoodsID).OrderBy(t => t.CommunityName).OrderBy(t => t.ResidenceName).OrderBy(t => t.StandardAddress).ToList();
                }
                //如果是分页查询，就分页返回
                else
                {
                    data1 = query.OrderBy(t => t.CountyID).OrderBy(t => t.NeighborhoodsID).OrderBy(t => t.CommunityName).OrderBy(t => t.ResidenceName).OrderBy(t => t.StandardAddress).Skip(PageSize * (PageNum - 1)).Take(PageSize).ToList();
                }

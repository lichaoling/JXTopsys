    ...
    public class RoadMPSearch
    {
        ...
        public static Dictionary<string, object> SearchRoadMP(int PageSize, int PageNum, string DistrictID, string CommunityName, string RoadName, string ShopName, string AddressCoding, string PropertyOwner, string StandardAddress, int MPNumberType, int UseState)
        {
            ...
            using (var dbContext = SystemUtils.NewEFDbContext)
            {
                ...
                {
                    data1 = query.OrderBy(t => t.CountyID).OrderBy(t => t.NeighborhoodsID).OrderBy(t => t.CommunityName).OrderBy(t => t.RoadName).OrderBy(t => t.MPNumber).ToList();
                }
                //如果是分页查询，就分页返回
                else
                {
                    data1 = query.OrderBy(t => t.CountyID).OrderBy(t => t.NeighborhoodsID).OrderBy(t => t.CommunityName).OrderBy(t => t.RoadName).OrderBy(t => t.MPNumber).Skip(PageSize * (PageNum - 1)).Take(PageSize).ToList();
                }

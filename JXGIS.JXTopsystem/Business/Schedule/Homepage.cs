        ...
        public static Dictionary<string, object> GetHomePageDetailData(DateTime start, DateTime end)
        {
            using (var dbContext = SystemUtils.NewEFDbContext)
            {
                ...
                mpsql1_yc += $@"union 
                                select b.ID from BG_MPOFCOUNTRY b 
                                left  join MPOFCOUNTRY a on b.MPID=a.ID 
                                where b.IsFinish=1 and b.checkuser='{LoginUtils.CurrentUser.UserID}' and b.sbly='{Enums.SBLY.yc}' and b.CreateTime>'{(start).ToString("yyyy/MM/dd")}' and b.CreateTime<'{(end.AddDays(1)).ToString("yyyy/MM/dd")}'
                                union 
                                select b.ID from BG_MPOFRESIDENCE b 
                                left  join MPOFRESIDENCE a on b.MPID=a.ID 
                                where b.IsFinish=1 and b.checkuser='{LoginUtils.CurrentUser.UserID}' and b.sbly='{Enums.SBLY.yc}' and b.CreateTime>'{(start).ToString("yyyy/MM/dd")}' and b.CreateTime<'{(end.AddDays(1)).ToString("yyyy/MM/dd")}'
                ...
                mpsql1_zxsb += $@"union 
                                select b.ID from BG_MPOFCOUNTRY b 
                                left  join MPOFCOUNTRY a on b.MPID=a.ID 
                                where b.IsFinish=1 and b.checkuser='{LoginUtils.CurrentUser.UserID}' and b.sbly='{Enums.SBLY.zxsb}' and b.CreateTime>'{(start).ToString("yyyy/MM/dd")}' and b.CreateTime<'{(end.AddDays(1)).ToString("yyyy/MM/dd")}'
                                union 
                                select b.ID from BG_MPOFRESIDENCE b 
                                left  join MPOFRESIDENCE a on b.MPID=a.ID 
                                where b.IsFinish=1 and b.checkuser='{LoginUtils.CurrentUser.UserID}' and b.sbly='{Enums.SBLY.zxsb}' and b.CreateTime>'{(start).ToString("yyyy/MM/dd")}' and b.CreateTime<'{(end.AddDays(1)).ToString("yyyy/MM/dd")}'
                ...
                mpsql1_zlb += $@"union 
                                select b.ID from BG_MPOFCOUNTRY b 
                                left  join MPOFCOUNTRY a on b.MPID=a.ID 
                                where b.IsFinish=1 and b.checkuser='{LoginUtils.CurrentUser.UserID}' and b.sbly='{Enums.SBLY.zlb}' and b.CreateTime>'{(start).ToString("yyyy/MM/dd")}' and b.CreateTime<'{(end.AddDays(1)).ToString("yyyy/MM/dd")}'
                                union 
                                select b.ID from BG_MPOFRESIDENCE b 
                                left  join MPOFRESIDENCE a on b.MPID=a.ID 
                                where b.IsFinish=1 and b.checkuser='{LoginUtils.CurrentUser.UserID}' and b.sbly='{Enums.SBLY.zlb}' and b.CreateTime>'{(start).ToString("yyyy/MM/dd")}' and b.CreateTime<'{(end.AddDays(1)).ToString("yyyy/MM/dd")}'
